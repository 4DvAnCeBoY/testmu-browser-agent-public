# testmu-browser-agent-public as MCP Server in Claude Code

This guide shows how to set up testmu-browser-agent-public as an MCP (Model Context Protocol) server so Claude Code can control a browser directly during your sessions.

## Prerequisites

- Claude Code installed
- testmu-browser-agent-public binary installed
...
```sh
mv testmu-browser-agent-public /usr/local/bin/
...
Claude will use the `testmu-browser-agent-public` MCP tools to open the page and return a screenshot.

## Troubleshooting

- Make sure `testmu-browser-agent-public` is on your `PATH` — Claude Code spawns it as a subprocess.
